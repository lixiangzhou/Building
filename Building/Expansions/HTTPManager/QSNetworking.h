 ...
 */
typedef void (^QSDownloadProgress)(int64_t bytesRead,
                                    int64_t totalBytesRead);

typedef QSDownloadProgress QSGetProgress;
typedef QSDownloadProgress QSPostProgress;
 ...
 */
typedef void (^QSUploadProgress)(int64_t bytesWritten,
                                  int64_t totalBytesWritten);

typedef NS_ENUM(NSUInteger, QSResponseType) {
  kQSResponseTypeJSON = 1, // 默认
  kQSResponseTypeXML  = 2, // XML
  // 特殊情况下，一转换服务器就无法识别的，默认会尝试转换成JSON，若失败则需要自己去转换
  kQSResponseTypeData = 3
};

typedef NS_ENUM(NSUInteger, QSRequestType) {
  kQSRequestTypeJSON = 1, // 默认
  kQSRequestTypePlainText  = 2 // 普通text/html
};

typedef NS_ENUM(NSInteger, QSNetworkStatus) {
    kQSNetworkStatusUnknown          = -1,//未知网络
    kQSNetworkStatusNotReachable     = 0,//网络无连接
    kQSNetworkStatusReachableViaWWAN = 1,//2，3，4G网络
    kQSNetworkStatusReachableViaWiFi = 2,//WIFI网络
};
...
typedef void(^QSResponseSuccess)(id response);
typedef void(^QSResponseFail)(NSError *error);
...
/**
 *
 *	取消某个请求。如果是要取消某个请求，最好是引用接口所返回来的QSURLSessionTask对象，
